...
# install maridb and send config file
...
#passwd value:smart
rabbitmqctl add_user openstack smart
...
ETCD_LISTEN_PEER_URLS="http://192.168.29.145:2380"
ETCD_LISTEN_CLIENT_URLS="http://192.168.29.145:2379"
ETCD_NAME="controller"
#[Clustering]
ETCD_INITIAL_ADVERTISE_PEER_URLS="http://192.168.29.145:2380"
ETCD_ADVERTISE_CLIENT_URLS="http://192.168.29.145:2379"
ETCD_INITIAL_CLUSTER="controller=http://192.168.29.145:2380"
...
USERNAME="root"
PASSWD="smart"
mysql -u$USERNAME -p$PASSWD -e "CREATE DATABASE keystone"
mysql -u$USERNAME -p$PASSWD -e "GRANT ALL PRIVILEGES ON keystone.* TO 'keystone'@'localhost' IDENTIFIED BY 'smart'"
mysql -u$USERNAME -p$PASSWD -e "GRANT ALL PRIVILEGES ON keystone.* TO 'keystone'@'%' IDENTIFIED BY 'smart'"

yum install openstack-keystone httpd mod_wsgi -y

sed -i '/^\[database\]/a\connection = mysql+pymysql://keystone:smart@controller/keystone' /etc/keystone/keystone.conf
...
keystone-manage bootstrap --bootstrap-password smart \
...
echo "export OS_PASSWORD=smart" >> /home/admin-openrc
...
USERNAME="root"
PASSWD="smart"
mysql -u$USERNAME -p$PASSWD -e "CREATE DATABASE glance"
mysql -u$USERNAME -p$PASSWD -e "GRANT ALL PRIVILEGES ON glance.* TO 'glance'@'localhost' IDENTIFIED BY 'smart'"
mysql -u$USERNAME -p$PASSWD -e "GRANT ALL PRIVILEGES ON glance.* TO 'glance'@'%' IDENTIFIED BY 'smart'"

openstack user create --domain default --password smart glance
...
sed -i '/^\[database\]/a\connection = mysql+pymysql://glance:smart@controller/glance' /etc/glance/glance-api.conf

sed -i '/^\[keystone_authtoken\]/a\password = smart'      /etc/glance/glance-api.conf
...
sed -i '/^\[database\]/a\connection = mysql+pymysql://glance:smart@controller/glance' /etc/glance/glance-registry.conf

sed -i '/^\[keystone_authtoken\]/a\password = smart'      /etc/glance/glance-registry.conf
...
USERNAME="root"
PASSWD="smart"
mysql -u$USERNAME -p$PASSWD -e  "CREATE DATABASE nova_api"
mysql -u$USERNAME -p$PASSWD -e  "CREATE DATABASE nova"
mysql -u$USERNAME -p$PASSWD -e  "CREATE DATABASE nova_cell0"
mysql -u$USERNAME -p$PASSWD -e  "CREATE DATABASE placement"
#grant privileges
mysql -u$USERNAME -p$PASSWD -e  "GRANT ALL PRIVILEGES ON nova_api.* TO 'nova'@'localhost' IDENTIFIED BY 'smart'"
mysql -u$USERNAME -p$PASSWD -e  "GRANT ALL PRIVILEGES ON nova_api.* TO 'nova'@'%' IDENTIFIED BY 'smart'"
mysql -u$USERNAME -p$PASSWD -e  "GRANT ALL PRIVILEGES ON nova.* TO 'nova'@'localhost' IDENTIFIED BY 'smart'"
mysql -u$USERNAME -p$PASSWD -e  "GRANT ALL PRIVILEGES ON nova.* TO 'nova'@'%' IDENTIFIED BY 'smart'"
mysql -u$USERNAME -p$PASSWD -e  "GRANT ALL PRIVILEGES ON nova_cell0.* TO 'nova'@'localhost' IDENTIFIED BY 'smart'"
mysql -u$USERNAME -p$PASSWD -e  "GRANT ALL PRIVILEGES ON nova_cell0.* TO 'nova'@'%' IDENTIFIED BY 'smart'"
mysql -u$USERNAME -p$PASSWD -e  "GRANT ALL PRIVILEGES ON placement.* TO 'placement'@'localhost' IDENTIFIED BY 'smart'"
mysql -u$USERNAME -p$PASSWD -e  "GRANT ALL PRIVILEGES ON placement.* TO 'placement'@'%' IDENTIFIED BY 'smart'"

#create user nova
openstack user create --domain default --password smart nova
...
openstack user create --domain default --password smart placement
...
sed -i '/^\[api_database\]/a\connection = mysql+pymysql://nova:smart@controller/nova_api' /etc/nova/nova.conf
sed -i '/^\[database\]/a\connection = mysql+pymysql://nova:smart@controller/nova' /etc/nova/nova.conf
sed -i '/^\[placement_database\]/a\connection = mysql+pymysql://placement:smart@controller/placement' /etc/nova/nova.conf
...
USERNAME="root"
PASSWD="smart"
mysql -u$USERNAME -p$PASSWD -e  "CREATE DATABASE neutron"
mysql -u$USERNAME -p$PASSWD -e  "GRANT ALL PRIVILEGES ON neutron.* TO 'neutron'@'localhost' IDENTIFIED BY 'smart'"
mysql -u$USERNAME -p$PASSWD -e  "GRANT ALL PRIVILEGES ON neutron.* TO 'neutron'@'%' IDENTIFIED BY 'smart'"
openstack user create --domain default --password smart  neutron
...
#neutron config file 
sed -i '/^\[database\]/a\connection = mysql+pymysql://neutron:smart@controller/neutron' /etc/neutron/neutron.conf
...
sed -i '/^allow_overlapping_ips/a\transport_url = rabbit://openstack:smart@controller' /etc/neutron/neutron.conf
...
sed -i '/^\[nova\]/a\password = smart' /etc/neutron/neutron.conf
...
sed -i '/^nova_metadata_host/a\metadata_proxy_shared_secret = smart' /etc/neutron/metadata_agent.ini

#set neutron section in nova.conf
sed -i '/^\[neutron\]/a\metadata_proxy_shared_secret = smart' /etc/nova/nova.conf
